...
def create(request):
    ...
    # form으로 전달받는 형태가 2가지
    # 1. GET요청 -> 비어있는 폼 전달
    # 2. 유효성 검증 실패 -> 에러메세지를 포함한 채로 폼 전달
    context = {'form' : form}
...
def update(request, article_pk):
  ...
  if request.user == article.user:
    ...
    return redirect('articles:index')
  # 2가지 form 형식
  # 1. GET -> 초기값 폼에 넣어서 사용자에게 던져줌
  # 2. POST -> is_vlaid가 False가 리턴되었을 때, 오류 메시지 포함해서 사용자에게 던져줌
  context = {
     'form' : form,
     'article' : article,
    }
...
def like(request, article_pk):
  ...
  # 현제 개시글을 좋아요 누른 사람 목록에서, 현재 접속한 유저가 있을 경우
  # -> 좋아요 취소
